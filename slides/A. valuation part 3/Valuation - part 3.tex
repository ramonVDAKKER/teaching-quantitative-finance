...
\begin{frame}{Roadmap for these slides}
The logic of risk-neutral pricing:
\begin{enumerate}
\item \textbf{First Fundamental Theorem:} no arbitrage $\Leftrightarrow$ there is a measure $\mathbb{Q}\sim\mathbb{P}$ under which \emph{discounted} prices are martingales.
\item To \emph{use} it we must actually find $\mathbb{Q}$ - this is what \textbf{Girsanov}'s theorem delivers (an intermezzo).
\item \textbf{Apply it:} under $\mathbb{Q}$ the stock earns $r$, and the option price is a discounted $\mathbb{Q}$-expectation. Worked examples (digital put, Philips put).
\item An equivalent encoding: the \textbf{pricing kernel}.
\item When is $\mathbb{Q}$ \emph{unique}? The \textbf{Second Fundamental Theorem} (completeness).
\end{enumerate}
\end{frame}
...
In part~1 we priced options in the one-period toy model two ways: by \emph{replication}, and by \emph{risk-neutral} pricing under a measure $\mathbb{Q}$ that made $S/B$ a martingale. There we proved the First Fundamental Theorem of asset pricing.
\medskip

The same ideas carry over to continuous time - only the machinery is heavier. We now
\begin{itemize}
\item state the continuous-time First Fundamental Theorem;
\item \emph{find} the risk-neutral measure $\mathbb{Q}$ (via Girsanov);
\item use it to price European options.
\end{itemize}
\end{frame}
...
\item a sketch of proof is given in the \hyperlink{app:girsanov}{appendix}
\end{itemize}
\end{frame}
...
\textbf{Recall the First Fundamental Theorem} (stated earlier): for a num\'eraire $N$, absence of arbitrage means there is a measure $\mathbb{Q}\sim\mathbb{P}$ under which \emph{every} discounted price $A/N$ is a martingale.
\medskip

\textbf{Plan.} In the Black-Scholes market we now \emph{construct} this $\mathbb{Q}$ explicitly - using Girsanov - and then read off option prices as discounted $\mathbb{Q}$-expectations.
\medskip

(We use the money-market account $B$ as num\'eraire; any asset that stays strictly positive would also do.)
\end{frame}


\begin{frame}{Using FFT and Girsanov: setup}
\begin{itemize}
...
\begin{frame}{Using FFT and Girsanov: finding $\mathbb{Q}$}
\[
\rd \frac{S_t}{B_t} =(\mu -r) \frac{S_t}{B_t} \rd t + \sigma \frac{S_t}{B_t} \rd W_t^{\mathbb{P}}
\]
\begin{itemize}
...
\]
yields $\mathbb{Q}\sim\mathbb{P}$ such that
...
\begin{frame}{From the binomial model to the market price of risk}
In part~1 the risk-neutral probability $q$ tilted the real-world $p$ just enough to make $S/B$ a martingale. In continuous time Girsanov does the same job continuously, through the \textbf{market price of risk}
\[ \gamma=\frac{\mu-r}{\sigma}. \]
\begin{itemize}
\item $\mu-r$ is the stock's \emph{excess return} over the riskless rate; dividing by $\sigma$ gives excess return \emph{per unit of risk} (a Sharpe ratio);
\item Girsanov removes exactly this drift, $\rd W^{\mathbb{Q}}_t=\gamma\,\rd t+\rd W^{\mathbb{P}}_t$, turning the stock's drift from $\mu$ into $r$;
\item discrete analogue (part~1): $q=\dfrac{B_1/B_0-s_d/S_0}{s_u/S_0-s_d/S_0}$ is the same tilt, one period at a time.
\end{itemize}
The larger the risk premium $\mu-r$ (or the smaller the risk $\sigma$), the larger the adjustment $\gamma$.
\end{frame}
...
\begin{frame}{The real-world and risk-neutral laws of $S_T$}
The change of measure $\mathbb{P}\to\mathbb{Q}$ shifts the stock's drift from $\mu$ to $r$ but leaves $\sigma$ unchanged. So $S_T$ is lognormal under both, with the \emph{same} spread but a different centre:
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=9cm,height=4.4cm,axis lines=left,xlabel={$S_T$},ylabel={density},
  xmin=45,xmax=205,ymin=0,ymax=0.022,ytick=\empty,
  legend pos=north east,legend style={font=\scriptsize,draw=none,fill=none},
  xlabel style={font=\small},ylabel style={font=\small}]
\addplot[blue,thick,smooth] coordinates {(45,0.00000) (50,0.00002) (55,0.00008) (60,0.00031) (65,0.00091) (70,0.00210) (75,0.00406) (80,0.00676) (85,0.00992) (90,0.01308) (95,0.01577) (100,0.01760) (105,0.01838) (110,0.01813) (115,0.01701) (120,0.01527) (125,0.01320) (130,0.01104) (135,0.00896) (140,0.00708) (145,0.00547) (150,0.00414) (155,0.00308) (160,0.00225) (165,0.00162) (170,0.00116) (175,0.00081) (180,0.00057) (185,0.00039) (190,0.00027) (195,0.00018) (200,0.00012) (205,0.00008)};
\addlegendentry{under $\mathbb{P}$ (drift $\mu$)}
\addplot[red,thick,smooth] coordinates {(45,0.00001) (50,0.00008) (55,0.00036) (60,0.00112) (65,0.00271) (70,0.00531) (75,0.00879) (80,0.01264) (85,0.01618) (90,0.01877) (95,0.02003) (100,0.01992) (105,0.01864) (110,0.01656) (115,0.01405) (120,0.01147) (125,0.00904) (130,0.00692) (135,0.00516) (140,0.00376) (145,0.00268) (150,0.00188) (155,0.00130) (160,0.00089) (165,0.00060) (170,0.00040) (175,0.00026) (180,0.00017) (185,0.00011) (190,0.00007) (195,0.00005) (200,0.00003) (205,0.00002)};
\addlegendentry{under $\mathbb{Q}$ (drift $r$)}
\end{axis}
\end{tikzpicture}
\end{center}
\vspace{-2mm}
{\small Here $S_0=100,\ \sigma=20\%,\ T=1,\ \mu=12\%,\ r=3\%$; then $\mathbb{E}_{\mathbb{P}}[S_T]=112.8$ but $\mathbb{E}_{\mathbb{Q}}[S_T]=103.0$. Same outcomes (equivalent measures!), different likelihoods.}
\end{frame}

\begin{frame}{Worked example: under $\mathbb{Q}$ the stock earns the riskless rate}
Under $\mathbb{Q}$, $\ S_T=S_t\,\e^{(r-\frac12\sigma^2)(T-t)+\sigma(W_T^{\mathbb{Q}}-W_t^{\mathbb{Q}})}$ with $W_T^{\mathbb{Q}}-W_t^{\mathbb{Q}}\distr\rN(0,T-t)$. Using $\mathbb{E}[\e^{\sigma Z'}]=\e^{\frac12\sigma^2(T-t)}$ for $Z'\distr\rN(0,T-t)$:
\[ \mathbb{E}_{\mathbb{Q}}[S_T\mid\mathcal{F}_t]=S_t\,\e^{(r-\frac12\sigma^2)(T-t)}\,\e^{\frac12\sigma^2(T-t)}=S_t\,\e^{r(T-t)}. \]
Equivalently $\mathbb{E}_{\mathbb{Q}}[S_T/B_T\mid\mathcal{F}_t]=S_t/B_t$: \textbf{$S/B$ is a $\mathbb{Q}$-martingale}, as it must be. The same computation under $\mathbb{P}$ gives $\mathbb{E}_{\mathbb{P}}[S_T\mid\mathcal{F}_t]=S_t\,\e^{\mu(T-t)}$ - the stock earns its true drift $\mu>r$, a \emph{risk premium}.
\\[3pt]
This is the continuous-time version of the binomial identity $\frac{1}{B_1}\mathbb{E}_{\mathbb{Q}}[S_1]=S_0$ from part~1.
\end{frame}

\begin{frame}{Using FFT and Girsanov: the pricing formula}
\small{
\begin{itemize}
\item the SDE
\[
...
\right]
\end{align*}
{\footnotesize (we may drop the conditioning: $S_t$ is known at time $t$ and $W_T^{\mathbb{Q}}-W_t^{\mathbb{Q}}$ is independent of $\mathcal{F}_t$)}
...
\end{align*}
\medskip
This is exactly the price the part-2 PDE approach gives for the digital put - the two methods agree.}
\end{itemize}
\end{frame}
%
...
\begin{frame}{Quiz}
\begin{quizbox}
In the Black-Scholes market with $\mu=10\%$, $r=2\%$, $\sigma=25\%$:
\begin{enumerate}
\item What is the market price of risk $\gamma$?
\item Write the SDE for $S$ under the risk-neutral measure $\mathbb{Q}$.
\item What is the distribution of $\log S_T$ under $\mathbb{Q}$ (given $S_0$)?
\item Is $\mathbb{E}_{\mathbb{Q}}[S_T]$ larger or smaller than $\mathbb{E}_{\mathbb{P}}[S_T]$?
\end{enumerate}
\end{quizbox}
\end{frame}
...
\begin{fftbox}
The risk-neutral measure $\mathbb{Q}$ is \emph{not} the real-world measure: it never describes the actual probabilities of price moves, and under $\mathbb{Q}$ every asset earns only $r$.
\begin{itemize}
\item In what sense, then, is $\mathbb{Q}$ ``fictional but useful''?
\item Two investors who disagree about the drift $\mu$ (hence about $\mathbb{P}$) still agree on the option price. Why does $\mu$ drop out?
\item If a second, \emph{untraded} source of risk is added, $\mathbb{Q}$ is no longer unique (Second FTAP). What breaks in the replication argument?
\end{itemize}
\end{fftbox}
\end{frame}
...
\end{itemize}
\smallskip
{\small Here $K=Z/N$ is the discounted likelihood ratio $\tfrac1N\,\tfrac{\rd\mathbb{Q}}{\rd\mathbb{P}}$ - the continuous-time version of the kernel $m=\tfrac{B_0}{B_1}\tfrac{\rd\mathbb{Q}}{\rd\mathbb{P}}$ from part~1.}
\end{frame}
...
$
\rd K_t=\mu^K_t \rd t + \sigma^K_t \rd W_t^{\mathbb{P}}
$
\item It\^{o}'s product rule yields
\begin{align*}
\rd K_t B_t&=K_t \rd B_t + B_t \rd K_t+ \rd [K,B]_t\\
&=( r K_t B_t + B_t \mu^K_t) \rd t + \ldots \rd W_t^{\mathbb{P}}
\end{align*}
...
&=\left(  K_t\mu S_t + S_t\mu^K_t+ \sigma \sigma^K_t S_t\right)   \rd t + \ldots \rd W_t^{\mathbb{P}}
\end{align*}
$K_tS_t$ martingale $\implies$ no drift $\implies$ $\sigma^K_t=-K_t(\mu-r)/\sigma$
\end{itemize}
So
\[
K_0=1,\quad \rd K_t=-rK_t \rd t  -\frac{\mu-r}{\sigma} K_t \rd W_t^{\mathbb{P}}
\]
...
\[
K_t=\exp\left\{
\left(-r-0.5\left(\frac{\mu-r}{\sigma}\right)^2 \right)t -\frac{\mu-r}{\sigma} W_t^{\mathbb{P}}
\right\}
\]
Fair price of a derivative, with payoff $C_T=h(S_T)$, based on these assets is given by
\[
C_0=\frac{1}{K_0}\mathbb{E}_{\mathbb{P}}[ K_T C_T ]
\]
\begin{itemize}
\item both $K_T$ and $S_T$ (and hence $K_T C_T=K_T h(S_T)$) are functions of $W_T^{\mathbb{P}}$, so the price is the expectation of a function of a $\rN(0,T)$ random variable
...
\right\},
\]
showing that $K_T$ takes large values for large, negative values of $W_T$. And such states-of-the-world
correspond to small values of $S_T(\omega)$
\end{itemize}
\end{frame}

\begin{frame}{One object, three encodings}
The risk-neutral measure, the pricing kernel, and state prices are three views of the \emph{same} pricing rule.
\begin{center}
\begin{tikzpicture}[every node/.style={font=\small}]
\node[draw,rounded corners,fill=blue!6] (Q) at (0,0) {risk-neutral measure $\mathbb{Q}$};
\node[draw,rounded corners,fill=orange!10] (K) at (6,0) {pricing kernel $K=Z/N$};
\node[draw,rounded corners,fill=green!10] (pi) at (3,-1.7) {state prices};
\draw[<->,thick,gray] (Q)--(K) node[midway,above,font=\scriptsize]{$Z=\tfrac{\rd\mathbb{Q}}{\rd\mathbb{P}}$};
\draw[<->,thick,gray] (Q)--(pi);
\draw[<->,thick,gray] (K)--(pi);
\end{tikzpicture}
\end{center}
The same price, three ways:
\[ C_t = N_t\,\mathbb{E}_{\mathbb{Q}}\!\Big[\tfrac{C_T}{N_T}\,\Big|\,\mathcal{F}_t\Big]
= \tfrac{1}{K_t}\,\mathbb{E}_{\mathbb{P}}[\,K_T C_T\mid\mathcal{F}_t\,]
= \int C_T\,\rd\pi . \]
In the part-1 toy model these were the measure $\mathbb{Q}$, the kernel $m$, and the Arrow-Debreu prices $\pi_u,\pi_d$.
\end{frame}
...
\begin{frame}{Three methods, one price}
For the European digital put (payoff $1\{S_T\le K\}$) all three methods give the \emph{same} price:
\begin{center}
...
method & price $C_0$\\
\hline
Black-Scholes PDE (part 2) & $\e^{-rT}\Phi(d_-)$\\
risk-neutral pricing (part 3) & $\e^{-rT}\Phi(d_-)$\\
pricing kernel (part 3) & $\e^{-rT}\Phi(d_-)$\\
\hline
\end{tabular}
\end{center}
with $\,d_-=\dfrac{\log(K/S_0)-(r-\frac12\sigma^2)T}{\sigma\sqrt{T}}$. The PDE, the risk-neutral measure, and the pricing kernel are three routes to the \emph{same} no-arbitrage price - exactly as in the binomial toy model of part~1.
\end{frame}

\appendix
\section{Appendix}

\begin{frame}{Appendix: sketch proof of Girsanov (1/2)}
\hypertarget{app:girsanov}{}
\begin{itemize}
...
\[
(W_{t_1}^{\mathbb{Q}} - W_{t_0}^{\mathbb{Q}},\dots,W_{t_n}^{\mathbb{Q}} - W^{\mathbb{Q}}_{t_{n-1}})\distr \rN(0_n, \Sigma)
\]
...
\begin{frame}{Appendix: sketch proof of Girsanov (2/2)}
We  have 
\begin{align*}
...
$\int_a^b f_u \rd W^{\mathbb{P}}_u \distr \rN( 0, \int_a^b f_u^2 \rd u)$ for deterministic functions $f$:
\begin{align*}
& \mathbb{E}_{\mathbb{P}}\left[
\e^{ 
-\int_{t_0}^{t_1} ( \gamma_u - a_1)  \rd W^{\mathbb{P}}_u
...
&\qquad \times \e^{\frac{1}{2}\left( a_1^2 (t_1-t_0) + a_2^2 (t_2- t_1)\right)
