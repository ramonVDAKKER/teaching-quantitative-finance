...
\begin{tabular}{rl}
$\Delta$\, delta & $\dfrac{\partial C}{\partial S}$ \\[8mm] \pause
$\nu$\, vega & $\dfrac{\partial C}{\partial \sigma}$ \\ [8mm] \pause
$\rho$\, rho & $\dfrac{\partial C}{\partial r}$ \\ [8mm] \pause
$\Theta$\, theta & $\dfrac{\partial C}{\partial t}$

\end{tabular} \pause
\hspace{1.5cm}
\begin{tabular}{rl}
$\Gamma$\, gamma & $\dfrac{\partial^2 C}{\partial S^2}$ \\[8mm] \pause
...
\end{itemize}
\medskip
\emph{In general we must differentiate an expectation with respect to a parameter - stated abstractly next.}
\end{frame}

\begin{frame}{Goal}
\textbf{Goal (the general problem):} \\
...
\[
f^\prime(x) = \lim_{\epsilon\to 0} \frac{ f(x+\epsilon) - f(x)}{\epsilon}\approx \frac{ f(x+\delta) - f(x)}{\delta} %
\]
for small number $\delta$
\item this suggests to approximate
$(d/d\theta) \mathbb{E}_{\theta,\eta} [ h(X; \theta, \gamma)]$
by
\emph{one-sided finite-difference estimate}:
$$
\frac{ \mathbb{E}_{\theta+\delta,\eta} [ h(X; \theta+\delta, \gamma)]
-  \mathbb{E}_{\theta,\eta} [ h(X; \theta, \gamma)]
}{\delta}
$$
where $\delta$ is a small step (we write $\delta$, rather than the usual $h$, to avoid confusion with the payoff function $h$)
\item
however, we cannot (or do not want to) compute
$\mathbb{E}_{\theta+\delta,\eta} [ h(X; \theta+\delta, \gamma)]$ and
...
\begin{itemize}
\item choose small number $\delta$
\item simulate $X_1^{\theta,\eta},\dots,X_n^{\theta,\eta}$ i.i.d. from
$P_{\theta,\eta}$
\item simulate $X_1^{\theta+\delta,\eta},\dots,X_n^{\theta+\delta,\eta}$ i.i.d. from
$P_{\theta+\delta,\eta}$
\item approximate
\[
\frac{d}{d\theta} \mathbb{E}_{\theta,\eta} [ h(X; \theta, \gamma)]
...
$$
\frac{\frac{1}{n}\sum_{i=1}^n h(X_i^{\theta+\delta,\eta}; \theta+\delta,\gamma) - \frac{1}{n}\sum_{i=1}^n h(X_i^{\theta,\eta}; \theta,\gamma)}{\delta}
$$
where $\delta$ is a small step
\end{itemize}
\end{frame}
...
\[
\hat{f}_n^\prime(\theta) = \frac{ \hat f_n(\theta+\delta) - \hat f_n(\theta)}{\delta}
\]
\item estimation error $\hat{f}_n^\prime(\theta) - f^\prime(\theta)$ can be written as:
\begin{align*}
\hat{f}_n^\prime(\theta) - f^\prime(\theta) &
= \left[
\frac{ f(\theta+\delta) - f(\theta)}{\delta} - f'(\theta)
\right] \\
&\quad + \left[ \hat{f}_n^\prime(\theta) - \frac{ f(\theta+\delta) - f(\theta)}{\delta} \right]
\end{align*}
\end{itemize}
\end{frame}
...
we obtain
\[
\text{MSE} = \left( \frac{ f(\theta+\delta) - f(\theta)}{\delta} - f'(\theta)
    \right)^2 +
 \frac{1}{\delta^2} \var\left(
    \hat f_n(\theta+\delta) -  \hat f_n(\theta)
    \right)
\]
\end{itemize}
\end{frame}
...
\[
f(\theta+\delta) = f(\theta) + f'(\theta)\delta+\frac{f''(\theta)}{2}\delta^2 +O\big(\delta^3\big)
\]
\item so
\[
\left( \frac{f(\theta+\delta)-f(\theta)}{\delta}
-f'(\theta) \right)^2 = O(\delta^2)
\]
\item to determine  \emph{variance}, we first consider case  that
$(X_1^{\theta,\eta},\dots,X_n^{\theta,\eta})$ are independent of
$(X_1^{\theta+\delta,\eta},\dots,X_n^{\theta+\delta,\eta})$
\begin{align*}
 \var\!\left[
  \hat f_n(\theta+\delta) -  \hat f_n(\theta)
\right]
 &= \frac{1}{n} \left( \var_{\theta+\delta,\eta}(
h(X_1^{\theta+\delta,\eta};\theta+\delta,\gamma))
 \right. \\
&\quad +\left.\var_{\theta,\eta}(
h(X_1^{\theta,\eta};\theta,\gamma))\right)
\end{align*}
\item we assume that $ \var_{\theta+\delta,\eta}(
h(X_1^{\theta+\delta,\eta};\theta+\delta,\gamma))$ is a (locally) bounded function of $\theta$
\item this implies that we have
\[
\text{MSE}  = \text{bias}^2 + \text{var} =O(\delta^2)  +  O\left(\frac{1}{n\delta^2}\right)
\]
\end{itemize}
\end{frame}
...
\text{MSE}  = \text{bias}^2 + \text{var} = O( \delta^2)  + O\left( \frac{1}{n\delta^2} \right)
\]
\item letting $\delta$ tend to $0$ while keeping $n$ constant will make the MSE
tend to $\infty$
\item choosing $\delta$ such that $\text{MSE}$ is minimal
leads to $\delta \propto n^{-1/4}$ and
$$
\text{MSE} = O\big(n^{-1/2}\big)
$$
\item recall that (standard) MC approximation of option price itself (i.e. $f(\theta)$) satisfies
$\text{MSE} = O(n^{-1})$
\item can we improve our rate?
\end{itemize}
\end{frame}

\begin{frame}{Bias--variance trade-off in the step size $\delta$}
For the one-sided estimate $\;\text{MSE}(\delta)=\underbrace{O(\delta^2)}_{\text{bias}^2}+\underbrace{O(1/(n\delta^2))}_{\text{variance}}$. Small $\delta$: small bias but exploding variance; large $\delta$: small variance but large bias. The optimum sits at $\delta^\star\propto n^{-1/4}$.
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=8.6cm,height=4.6cm,axis lines=left,xlabel={step $\delta$},ylabel={MSE (a.u.)},
  xmin=0.04,xmax=0.32,ymin=0,ymax=10,xlabel style={font=\small},ylabel style={font=\small},
  legend pos=north east,legend style={font=\scriptsize,draw=none,fill=none}]
\addplot[blue,thick] coordinates {(0.040,0.160) (0.050,0.250) (0.060,0.360) (0.070,0.490) (0.080,0.640) (0.090,0.810) (0.100,1.000) (0.110,1.210) (0.120,1.440) (0.130,1.690) (0.140,1.960) (0.150,2.250) (0.160,2.560) (0.170,2.890) (0.180,3.240) (0.190,3.610) (0.200,4.000) (0.210,4.410) (0.220,4.840) (0.230,5.290) (0.240,5.760) (0.250,6.250) (0.260,6.760) (0.270,7.290) (0.280,7.840) (0.290,8.410) (0.300,9.000) (0.310,9.610) (0.320,10.240)}; \addlegendentry{bias$^2$}
\addplot[red,thick,dashed] coordinates {(0.040,6.250) (0.050,4.000) (0.060,2.778) (0.070,2.041) (0.080,1.562) (0.090,1.235) (0.100,1.000) (0.110,0.826) (0.120,0.694) (0.130,0.592) (0.140,0.510) (0.150,0.444) (0.160,0.391) (0.170,0.346) (0.180,0.309) (0.190,0.277) (0.200,0.250) (0.210,0.227) (0.220,0.207) (0.230,0.189) (0.240,0.174) (0.250,0.160) (0.260,0.148) (0.270,0.137) (0.280,0.128) (0.290,0.119) (0.300,0.111) (0.310,0.104) (0.320,0.098)}; \addlegendentry{variance}
\addplot[black,very thick] coordinates {(0.040,6.410) (0.050,4.250) (0.060,3.138) (0.070,2.531) (0.080,2.203) (0.090,2.045) (0.100,2.000) (0.110,2.036) (0.120,2.134) (0.130,2.282) (0.140,2.470) (0.150,2.694) (0.160,2.951) (0.170,3.236) (0.180,3.549) (0.190,3.887) (0.200,4.250) (0.210,4.637) (0.220,5.047) (0.230,5.479) (0.240,5.934) (0.250,6.410) (0.260,6.908) (0.270,7.427) (0.280,7.968) (0.290,8.529) (0.300,9.111) (0.310,9.714) (0.320,10.338)}; \addlegendentry{MSE}
\addplot[gray,dotted,thick] coordinates {(0.1,0) (0.1,10)};
\node[font=\scriptsize,gray,anchor=south] at (axis cs:0.1,0.2){$\delta^\star$};
\end{axis}
\end{tikzpicture}
\end{center}
\vspace{-2mm}
{\small Common random numbers remove the variance blow-up - then the MSE is $O(n^{-1})$ for any small $\delta$.}
\end{frame}
...
\[
h(X_i^{\theta,\eta};\theta,\gamma)
\stackrel{d}{=} g(Z_i;\theta) \text{ and }
h(X_i^{\theta+\delta,\eta};\theta+\delta,\gamma)\stackrel{d}{=} g(Z_i;\theta+\delta)
\]
\end{itemize}
...
g(Z_1;\theta+\delta) - g(Z_1;\theta)
\right)
 \\
= \frac{1}{n}  \var\!\Big( \frac{\partial g}{\partial \theta} (Z_1,\theta)\delta + O\big(\delta^2\big)\Big)
= \frac{1}{n}O\big(\delta^2\big) \hspace{1cm}
\end{align*}
which yields
\[
\text{MSE} = O( \delta^2 ) + O(  \frac{1}{n})
\]
\end{itemize}
\end{frame}
...
is of the form
$$
\text{MSE} = O( \delta^2) + O\left(\frac{1}{n}\right)\,. \hspace{2cm}
$$
so, for fixed $n$, the variance does not explode as $\delta$ becomes small
\item
bias is purely controlled by $\delta$ and variance by $n$
\item so we can take
$\delta$ as small as we want, and the convergence
...
\[
\frac{f(\theta+\delta) - f(\theta -\delta)    }{2\delta}
\]
\item from Taylor expansion $f(\theta+\delta)=f(\theta) + f'(\theta)\delta
+.5f''(\theta)\delta^2 + O(\delta^3)$  we see that bias is reduced
from  $O\big(\delta\big)$ to $O\big(\delta^2\big)$
...
\begin{align*}
f''(\theta) &\approx
\frac{ f'(\theta+\delta)  - f'(\theta)}{\delta}\approx
\frac{\frac{ f(\theta+\delta) -f(\theta)}{\delta} -
\frac{ f(\theta) -f(\theta-\delta)}{\delta}
}{\delta}\\
&
= \frac{f(\theta+\delta) - 2f(\theta) + f(\theta-\delta) }{\delta^2}
\end{align*}
...
\begin{frame}{Quiz}
\begin{quizbox}
You must estimate, by Monte Carlo, the \textbf{delta} of (a) a European call and (b) a digital call.
\begin{enumerate}
\item Which of bump-and-reprice / pathwise / likelihood-ratio work for each, and which fails?
\item For the one-sided bump estimate with independent samples (no common random numbers), what is the optimal step $\delta$ as a function of $n$, and the resulting MSE rate?
\end{enumerate}
\end{quizbox}
\end{frame}
...
\begin{frame}{Core idea Likelihood Ratio Method (LRM)}
Suppose $X \sim N(\mu,\sigma^2)$ and we wish to compute the derivative of $\mathbb{E}_{\mu,\sigma}[f(X)]$ w.r.t $\mu$. Denote by $\phi_{\mu,\sigma}$ the density of $N(\mu,\sigma^2)$.
\vskip4mm
\textbf{LRM:}
\begin{align*}
\frac{\partial}{\partial\mu} \mathbb{E}_{\mu,\sigma}[f(X)] &= \frac{\partial}{\partial\mu} \int f(x) \phi_{\mu,\sigma}(x) dx \stackrel{?}{=} \int f(x) \frac{\frac{\partial  \phi_{\mu,\sigma}}{\partial\mu}(x)}{\phi_{\mu,\sigma}(x)} \phi_{\mu,\sigma}(x) dx
\\
...
$$
\frac{\partial}{\partial \theta} \mathbb{E} [f(Z,\theta)] = \mathbb{E} \Big[ \frac{\partial}{\partial\theta}f(Z,\theta)\Big].
$$
...
\]
Intuitively, this is what we need here: A function that behaves like the (non-existent) derivative of $f$ when it is inside an integral.
Such functions are called \emph{absolutely continuous}.
You are implicitly aware of this from probability theory:
...
\begin{tabular}{lccc}
\hline
 & bump \& reprice & pathwise & LRM\\
\hline
bias & yes & none & none\\
applies to & \emph{any} payoff & smooth only & \emph{any} payoff\\
needs & - & payoff deriv. & density score\\
variance & high (low w/ CRN) & low & can be high\\
\hline
\end{tabular}}
\\[2pt]{\scriptsize Bump variance is $O(1/(n\delta^2))$ with independent samples (it explodes as $\delta\to0$) but $O(1/n)$ with common random numbers; its bias is $O(\delta)$ one-sided, $O(\delta^2)$ two-sided.}
\end{center}
\medskip
Rule of thumb: \emph{pathwise} for smooth payoffs (unbiased, low variance); \emph{likelihood ratio} for discontinuous payoffs such as the digital; \emph{bump-and-reprice} as the always-applicable fallback and the easy route to second-order Greeks.
\end{frame}

\begin{frame}{Food for thought}
\begin{fftbox}
\begin{itemize}
\item The pathwise method differentiates the \emph{payoff} - exactly what modern \emph{automatic differentiation} does mechanically, and how Greeks are computed at scale today. The likelihood-ratio method differentiates the \emph{density}; \emph{Malliavin calculus} generalizes it. Which would you reach for on a \emph{barrier} option?
\item Bump-and-reprice has both a bias and a variance, whereas plain MC pricing has only variance - yet with common random numbers bump-and-reprice recovers the $O(n^{-1})$ rate. Why does \emph{sharing the randomness} matter so much?
\end{itemize}
\end{fftbox}
\end{frame}
