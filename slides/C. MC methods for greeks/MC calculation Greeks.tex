...
\begin{frame}{Bias--variance trade-off in the step size $h$}
For the one-sided estimate $\;\text{MSE}(h)=\underbrace{O(h^2)}_{\text{bias}^2}+\underbrace{O(1/(nh^2))}_{\text{variance}}$. Small $h$: small bias but exploding variance; large $h$: small variance but large bias. The optimum sits at $h^\star\propto n^{-1/4}$.
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=8.6cm,height=4.6cm,axis lines=left,xlabel={step $h$},ylabel={MSE (a.u.)},
...
\node[font=\scriptsize,gray,anchor=south] at (axis cs:0.1,0.2){$h^\star$};
\end{axis}
\end{tikzpicture}
\end{center}
\vspace{-2mm}
{\small Common random numbers remove the variance blow-up --- then the MSE is $O(n^{-1})$ for any small $h$.}
\end{frame}
...
h(X_i^{\theta+h,\eta};\theta+h,\gamma)\stackrel{d}{=} g(Z_i;\theta+h)
\]
\end{itemize}
...
\item For the one-sided bump estimate with independent samples (no common random numbers), what is the optimal step $h$ as a function of $n$, and the resulting MSE rate?
\end{enumerate}
\end{quizbox}
\end{frame}
...
\hline
\end{tabular}}
\\[2pt]{\scriptsize Bump variance is $O(1/(nh^2))$ with independent samples (it explodes as $h\to0$) but $O(1/n)$ with common random numbers; its bias is $O(h)$ one-sided, $O(h^2)$ two-sided.}
\end{center}
\medskip
